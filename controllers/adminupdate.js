...
exports.updateAdminWallet = (req, res, next) => {
    AdminQuery.findOne({email: 'admin@theglobalasset.com'}, (err, user) => {
        if(user === null) {
            ...
            AdminQuery.findOneAndUpdate({email: 'admin@theglobalasset.com'}, {walletAddress: req.body.newAddress}, {useFindAndModify: false, new: true}, (err, resp) => {
                if(err){
                    res.status(400).json({
    ...
    AdminQuery.findOne({email: 'admin@theglobalasset.com'}, (err, user) => {
        if (user === null) {
            ...
            if (user.withdrawStatus) {
                AdminQuery.findOneAndUpdate({email: 'admin@theglobalasset.com'}, {withdrawStatus: false}, {useFindAndModify: false, new: true}, (err, resp) => {
                    if(err){
                        res.status(400).json({
                ...
                AdminQuery.findOneAndUpdate({email: 'admin@theglobalasset.com'}, {withdrawStatus: true}, {useFindAndModify: false, new: true}, (err, resp) => {
                    if(err){
                        res.status(400).json({
                            message: "An error occured when changing withdraw status",
                        })
